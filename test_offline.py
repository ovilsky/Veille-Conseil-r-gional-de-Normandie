...
def run():
    with patch.object(v.SESSION, "get", side_effect=fake_get), patch("time.sleep", return_value=None):
        ...
        assert len(elus_rne) == 4, f"attendu 4 élus normands (filtre région), obtenu {len(elus_rne)} : {elus_rne}"
        noms_rne = {e["nom"] for e in elus_rne}
        assert noms_rne == {"Sophie GAUGAIN", "Clotilde EUDIER", "Hervé MORIN", "Alex INTROUVABLE"}, noms_rne
        ...
        # lien "probable" construit à partir du nom, jamais vérifié, mais
        # disponible même sans scraping réussi de normandie.fr
        assert gaugain_rne["url_probable"] == "https://www.normandie.fr/sophie-gaugain", gaugain_rne
        assert v.slugify_name_normandie("Sophie Gaugain") == "sophie-gaugain"
        assert v.slugify_name_normandie("Fabien Achard de Leluardière") == "fabien-achard-de-leluardiere"
        print("✓ Lien 'probable' vers la fiche normandie.fr construit à partir du nom RNE")
        ...
        print(f"✓ RNE : {len(elus_rne)} élus normands filtrés sur {5} lignes source, colonnes tabulées reconnues, fonction OK")
        ...
        assert len(elus_final) == 4, "le roster RNE ne doit jamais être réduit par l'enrichissement"
        ...
        assert gaugain_final["groupe_source"] == "auto", \
            "un groupe rempli par le vrai scraping doit être marqué 'auto', pas confondu avec le repli manuel"
        ...
        with patch.object(v.SESSION, "get", side_effect=fake_get_elus_blocked):
            ...
            # Gaugain, Morin et Eudier sont dans MANUAL_ENRICHMENT_SNAPSHOT
            # (repli) : ils doivent récupérer un groupe malgré le 403, mais
            # marqué explicitement comme non-automatique (jamais confondu
            # avec un vrai scraping frais).
            gaugain_blocked = next(e for e in elus_blocked if e["nom_famille"] == "GAUGAIN")
            assert gaugain_blocked["groupe"] == "La Normandie Conquérante avec Hervé Morin"
            assert gaugain_blocked["groupe_source"] and gaugain_blocked["groupe_source"] != "auto"
            morin_blocked = next(e for e in elus_blocked if e["nom_famille"] == "MORIN")
            assert morin_blocked["groupe"] == "La Normandie Conquérante avec Hervé Morin"
            assert morin_blocked["groupe_source"] and morin_blocked["groupe_source"] != "auto"
            eudier_blocked = next(e for e in elus_blocked if e["nom_famille"] == "EUDIER")
            assert eudier_blocked["groupe"] == "La Normandie Conquérante avec Hervé Morin"
            assert eudier_blocked["groupe_source"] and eudier_blocked["groupe_source"] != "auto"
            # "Alex Introuvable" n'est délibérément dans aucune source
            # (ni enrichissement, ni repli manuel) : son groupe doit rester
            # None plutôt que d'inventer une valeur.
            introuvable_blocked = next(e for e in elus_blocked if e["nom_famille"] == "INTROUVABLE")
            assert introuvable_blocked["groupe"] is None, \
                "sans enrichissement automatique ni entrée de repli, le groupe doit rester None"
        print("✓ Résilience confirmée : 403 sur normandie.fr n'affecte plus le roster RNE (0 élu perdu)")
        print("✓ Repli manuel (MANUAL_ENRICHMENT_SNAPSHOT) appliqué correctement, sans écraser l'auto ni inventer de valeur")
